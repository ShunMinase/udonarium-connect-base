...
// Link版のプロパティ
interface GhostLinkProps extends BaseProps {
  as: 'link'
  href: string
  disabled?: boolean
  target?: string
...
// Button版のプロパティ
interface GhostButtonProps extends BaseProps {
  as: 'button'
...
type GhostProps = GhostLinkProps | GhostButtonProps

const Ghost: React.FC<GhostProps> = (props) => {
  const {
    size = 'md',
    width,
    colorScheme = 'white',
  ...
  const styledClasses = getStyledClasses('ghost', size, width, isDisabledOrLoading, colorScheme, className)
